...
On `main`, workflows use `--from . gha-utils` to run the CLI from local source (dogfooding). The freeze commit freezes these to `'gha-utils==X.Y.Z'` so tagged releases reference a published package. The unfreeze commit reverts them back for the next development cycle.

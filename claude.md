...
- **Sphinx extension order:** In `docs/conf.py`, `repomatic.myst_docstrings` must be registered before `sphinx_autodoc_typehints`. Both hook `autodoc-process-docstring` at default priority 500, so registration order determines execution order. MyST→reST conversion must run first on the original docstring; then `sphinx_autodoc_typehints` injects type annotations as valid reST.
...
- **Parameter and return documentation:** Use reST field lists (`:param name:`, `:return:`) for function/method arguments and return values. These pass through the MyST→reST converter unchanged and are processed directly by `sphinx.ext.autodoc`. Use `:return:` (not `:returns:`). Continuation lines are indented to align with the description text above.

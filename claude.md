...
- **`[tool.repomatic]` configuration table in `readme.md`**: Generated by `repomatic --table-format github config` from the `Config` dataclass docstrings. Re-run the command and update the pasted table when adding, removing, or modifying config fields.
- **PAT permissions**: The `REQUIRED_PAT_PERMISSIONS` constant in `repomatic/github/token.py` is the single source of truth for required fine-grained PAT permissions. When adding, removing, or changing a required permission, update all of these to match:
  1. `REQUIRED_PAT_PERMISSIONS` in `repomatic/github/token.py` (constant and module docstring).
  2. The permission table in `repomatic/templates/setup-guide.md`.
  3. The pre-filled token creation URL query parameters in `setup-guide.md`.
  4. The PAT capability check functions in `repomatic/lint_repo.py` (one check per testable permission).
  5. The `lint-repo` CLI docstring in `repomatic/cli.py`.
  6. The `lint-repo` job description in `readme.md`.
- **Repository configuration expectations**: The `lint-repo` job enforces repo settings described in the setup guide (Dependabot config absent, security updates disabled, Renovate config exists, vulnerability alerts enabled). When adding new setup steps to the guide, add a corresponding check to `run_repo_lint()` in `repomatic/lint_repo.py`. If the check cannot be automated (e.g., branch rulesets require `administration:read`), document the limitation in a comment.
- **PAT permission review**: When adding or removing workflow jobs that use `REPOMATIC_PAT`, or changing what API operations those jobs perform, review `REQUIRED_PAT_PERMISSIONS` in `repomatic/github/token.py` to verify the permission set is still minimal and complete. Permissions should follow the principle of least privilege — do not request permissions that no job uses, and do not omit permissions that a job depends on. Check the `secrets.REPOMATIC_PAT` references across all workflow files to audit actual usage.
...
- **Maintenance**: `/awesome-triage`, `/repomatic-audit`, `/repomatic-topics`, `/sphinx-docs-sync`

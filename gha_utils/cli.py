...
def get_header(ctx: Context):
    """Generates metadata to be leaved as comments to the top of a file generated by
    this CLI.
    """
    return (
...
@gha_utils.command(short_help="Update Git's .mailmap file with all contributors")
@option(
    "--source",
    type=path(exists=True, readable=True, resolve_path=True),
    default=".mailmap",
...
@pass_context
def mailmap(ctx, source, destination_mailmap):
    """Update a ``.mailmap`` file with all missing contributors found in Git commit
    history.

    By default the existing .mailmap is read to be used as initial mapping. To which
    missing contributors are added. Then the results are printed to the console output.
    So `gha-utils mailmap` is the same as a call to
    `gha-utils mailmap --source .mailmap -`.

    To have the updated results written to a file on disk, specify the output file like
    so: `gha-utils mailmap .mailmap`.

    The updated results are quite dumb, so it is advised to identify potential
    duplicate identities, then regroup them by hand.
    """
    initial_content = None
    if source:

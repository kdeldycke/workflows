...
"""Tests for the ``sync-linter-configs`` CLI command."""
...
from repomatic.cli import sync_linter_configs
...
def test_sync_linter_configs_writes_canonical_config(
    ...
    result = CliRunner().invoke(sync_linter_configs)
...
def test_sync_linter_configs_config_toggle_off(tmp_path, monkeypatch):
    """Exit 0 when ``linter-sync = false`` in ``[tool.repomatic]``."""
    monkeypatch.chdir(tmp_path)
    target = tmp_path / "zizmor.yaml"
    target.write_text("# old content")

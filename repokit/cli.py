...
def generate_header(ctx: Context) -> str:
    ...
    header = (
        ...
        " - https://github.com/kdeldycke/repokit\n"
...
def init_project(
    ...
    """Bootstrap a repository to use reusable workflows from kdeldycke/repokit.
    ...
    \b
        # Multiple components
        repokit init ruff bumpversion

    \b
        # Overwrite existing files
        repokit init --overwrite workflows
    """
    result = run_init(
...
@repokit.command(short_help="Maintain a Markdown-formatted changelog")
@option(
    "--source",
...
@repokit.command(short_help="Prepare files for a release")
@option(
    "--changelog",
    "changelog_path",
...
def release_prep(
    ...
    workflows repository (kdeldycke/repokit).
    ...
    \b
        ...
        repokit release-prep

    \b
        # In GitHub Actions on kdeldycke/repokit (auto-detects --update-workflows)
        repokit release-prep

    \b
        # Post-release: retarget workflows to main branch
        repokit release-prep --post-release
    """
...
@repokit.command(short_help="Check if a version bump is allowed")
@option(
    "--part",
...
def version_check(part: str) -> None:
    ...
    Examples:
        ...
        repokit version-check --part minor

        # Check if major version bump is allowed
        repokit version-check --part major
    ...
    Use in GitHub Actions:
        allowed=$( repokit version-check --part minor )
...
@repokit.command(short_help="Sync .gitignore from gitignore.io templates")
@option(
    "--output",
    "output_path",
    type=file_path(writable=True, resolve_path=True, allow_dash=True),
    default=None,
    help=("Output path. Defaults to gitignore-location from [tool.repokit] config."),
)
def sync_gitignore(output_path: Path | None) -> None:
    ...
    ``[tool.repokit]`` config, then appends ``gitignore-extra-content``.
    ...
    Examples:
        ...
        repokit sync-gitignore

    \b
        # Write to custom location
        repokit sync-gitignore --output ./custom/.gitignore

    \b
        # Preview on stdout
        repokit sync-gitignore --output -
    """
    ...
    request = Request(url, headers={"User-Agent": f"repokit/{__version__}"})  # noqa: S310
...
def _apply_workflow_config(
    ...
    """Apply ``[tool.repokit]`` config filtering to workflow names.
...
@repokit.group(short_help="Manage downstream workflow caller files")
def workflow():
    """Manage downstream workflow caller files.

    Generate, synchronize, and lint thin caller workflows that delegate to
    the canonical reusable workflows in ``kdeldycke/repokit``.
...
def create(ctx, output_format, version, repo, output_dir, workflow_names):
    ...
    Examples:
        ...
        repokit workflow create

    \b
        # Generate specific workflows
        repokit workflow create release.yaml lint.yaml

    \b
        # Generate with a pinned version
        repokit workflow create --version v5.8.0

    \b
        # Full copy mode
        repokit workflow create --format full-copy
    """
    fmt = WorkflowFormat(output_format)
    filtered = _apply_workflow_config(workflow_names, fmt)
    if filtered is None:
        logging.info(
            "[tool.repokit] workflow-sync is disabled. Skipping workflow create."
        )
        ctx.exit(0)
...
def sync(ctx, output_format, version, repo, output_dir, workflow_names):
    ...
    Examples:
        ...
        repokit workflow sync --version v5.9.0

    \b
        # Sync specific workflows
        repokit workflow sync release.yaml lint.yaml
    """
    fmt = WorkflowFormat(output_format)
    filtered = _apply_workflow_config(workflow_names, fmt)
    if filtered is None:
        logging.info(
            "[tool.repokit] workflow-sync is disabled. Skipping workflow sync."
        )
        ctx.exit(0)
...
def lint(ctx, workflow_dir, repo, fatal):
    ...
    Examples:
        ...
        repokit workflow lint

    \b
        # Lint with fatal mode (exit 1 on issues)
        repokit workflow lint --fatal

    \b
        # Lint a custom directory
        repokit workflow lint --workflow-dir ./my-workflows
    """
...
@repokit.command(short_help="Sync Git's .mailmap file with missing contributors")
@option(
    "--source",
...
@repokit.command(short_help="Run a test plan from a file against a binary")
@option(
    "--command",
    "--binary",
    required=True,
...
def test_plan(
    ...
    # Load [tool.repokit] config for fallback values.
    config = load_repokit_config()
    ...
    if plan_file or plan_envvar:
        ...
        # Fall back to [tool.repokit] config.
        config_test_plan = config.get("test-plan")
        if config_test_plan:
            logging.info("Get test plan from [tool.repokit] test-plan config.")
        ...
        if not test_list:
            logging.warning(
                ...
                " [tool.repokit] config: use default test plan."
            )
...
@repokit.command(short_help="Label issues/PRs from GitHub sponsors")
@option(
    "--owner",
...
def sponsor_label(
    ...
    Examples:
        ...
        repokit sponsor-label

    \b
        # Override specific values
        repokit sponsor-label --label "sponsor"

    \b
        # Manual invocation with all values
        repokit sponsor-label --owner kdeldycke --author some-user \\
            --repo kdeldycke/repokit --number 123 --issue
    """
...
@option(
    ...
    type=file_path(writable=True, resolve_path=True, allow_dash=True),
    default=None,
    help="Output file path. Defaults to [tool.repokit] config or stdout.",
)
def deps_graph(
    package: str | None,
    groups: tuple[str, ...],
    ...
    Examples:
        ...
        repokit update-deps-graph

    \b
        # Include test dependencies
        repokit update-deps-graph --group test

    \b
        # Include all groups and extras
        repokit update-deps-graph --all-groups --all-extras

    \b
        # Include all groups except typing
        repokit update-deps-graph --all-groups --no-group typing

    \b
        # Include all extras except one
        repokit update-deps-graph --all-extras --no-extra json5

    \b
        # Show only test group dependencies (no main deps)
        repokit update-deps-graph --only-group test

    \b
        # Show only a specific extra's dependencies
        repokit update-deps-graph --only-extra xml

    \b
        # Focus on a specific package
        repokit update-deps-graph --package click-extra

    \b
        # Limit graph depth to 2 levels
        repokit update-deps-graph --level 2

    \b
        # Save to file
        repokit update-deps-graph --output docs/dependency-graph.md
    """
    config = load_repokit_config()
...
@repokit.command(short_help="Manage broken links issue lifecycle")
@option(
    "--lychee-exit-code",
    type=int,
    default=None,
...
def broken_links(
    ...
    Examples:
        ...
        repokit broken-links --lychee-exit-code 2

    \b
        # Explicit options
        repokit broken-links \\
            --lychee-exit-code 2 \\
...
@repokit.command(short_help="Manage setup guide issue lifecycle")
@option(
    "--has-pat",
    is_flag=True,
    default=False,
...
@repokit.command(short_help="Verify binary architecture using exiftool")
@option(
    "--target",
...
def verify_binary(target: str, binary_path: Path) -> None:
    ...
    Examples:
        ...
        repokit verify-binary --target linux-arm64 --binary ./mpm-linux-arm64.bin

    \b
        # Verify a Windows x64 binary
        repokit verify-binary --target windows-x64 --binary ./mpm-windows-x64.exe
    """
    verify_binary_arch(target, binary_path)
    echo(f"Binary architecture verified for {target}: {binary_path}")


@repokit.command(short_help="Re-lock and revert if only timestamp noise changed")
@option(
    "--lockfile",
...
def sync_uv_lock_cmd(lockfile: Path) -> None:
    ...
    Examples:
        ...
        repokit sync-uv-lock

    \b
        # Check a different lock file
        repokit sync-uv-lock --lockfile path/to/uv.lock
    """
...
@repokit.command(short_help="Sync bumpversion config from bundled template")
def sync_bumpversion() -> None:
    ...
    bundled in ``repokit``. Designed for the ``sync-bumpversion`` autofix job.
    The ``repokit init bumpversion`` command remains available for interactive
    bootstrapping.
    """
    result = run_init(
        output_dir=Path("."),
...
@repokit.command(short_help="Sync linter config files from bundled definitions")
def sync_linter_configs() -> None:
    """Sync linter configuration files from the bundled definitions in ``repokit``.

    Overwrites ``.github/zizmor.yml`` with the canonical configuration
    bundled in ``repokit``. Designed for the ``sync-linter-configs`` autofix job.
    Use ``repokit init linters`` for interactive bootstrapping.
    """
    result = run_init(
        output_dir=Path("."),
        components=("linters",),
...
@repokit.command(short_help="Sync Claude Code skills from bundled definitions")
def sync_skills() -> None:
    """Sync Claude Code skill files from the bundled definitions in ``repokit``.

    Overwrites ``.claude/skills/`` with the canonical skill definitions
    bundled in ``repokit``. Designed for the ``sync-skills`` autofix job.
    Use ``repokit init skills`` for interactive bootstrapping.
    """
    result = run_init(
        output_dir=Path("."),
        components=("skills",),
...
@repokit.command(short_help="Sync Renovate config from canonical reference")
@option(
    "--output",
    "output_path",
...
def sync_renovate(ctx: Context, output_path: Path) -> None:
    ...
    ``repokit``, which strips repo-specific settings (``customManagers``,
    ``assignees``) from the upstream ``kdeldycke/repokit`` reference.
    ...
    if not config.get("renovate-sync", True):
        logging.info(
            "[tool.repokit] renovate-sync is disabled. Skipping Renovate config sync."
        )
        ctx.exit(0)
...
@repokit.command(short_help="Check Renovate migration prerequisites")
@option(
    "--repo",
    default=None,
    help="Repository in 'owner/repo' format. Defaults to $GITHUB_REPOSITORY.",
...
def check_renovate(
    ...
    Examples:
        ...
        repokit check-renovate

    \b
        # JSON output for parsing
        repokit check-renovate --format=json

    \b
        # GitHub Actions output format
        repokit check-renovate --format=github --output "$GITHUB_OUTPUT"

    \b
        # Manual invocation
        repokit check-renovate --repo owner/repo --sha abc123
    """
...
@repokit.command(short_help="Run repository consistency checks")
@option(
    "--repo-name",
    default=None,
...
def lint_repo(
    ...
    Examples:
        ...
        repokit lint-repo --repo-name my-package

    \b
        # Local run (derives repo from $GITHUB_REPOSITORY or --repo)
        repokit lint-repo --repo owner/repo
    """
    # Apply defaults from environment.
    if repo is None:
        repo = os.getenv("GITHUB_REPOSITORY", "")
...
@repokit.command(short_help="Check changelog dates against release dates")
@option(
    "--changelog",
    "changelog_path",
    type=file_path(exists=True, readable=True, resolve_path=True),
...
def lint_changelog(
    ...
    Examples:
        ...
        repokit lint-changelog

    \b
        # Fix dates and add admonitions
        repokit lint-changelog --fix

    \b
        # Explicit package name
        repokit lint-changelog --package repokit
    """
    exit_code = lint_changelog_dates(changelog_path, package=package, fix=fix)
    ctx.exit(exit_code)


@repokit.command(short_help="Create and push a Git tag")
@option(
    "--tag",
    required=True,
...
def git_tag(
    ...
    Examples:
        ...
        repokit git-tag --tag v1.2.3

    \b
        # Tag a specific commit
        repokit git-tag --tag v1.2.3 --commit abc123def

    \b
        # Create tag without pushing
        repokit git-tag --tag v1.2.3 --no-push

    \b
        # Fail if tag exists
        repokit git-tag --tag v1.2.3 --error-existing

    \b
        # Output result for GitHub Actions
        repokit git-tag --tag v1.2.3 --output "$GITHUB_OUTPUT"
    """
    try:
...
@repokit.command(short_help="Generate PR body with workflow metadata")
@option(
    "--prefix",
...
def pr_body(
    ...
    \b
        ...
        repokit pr-body --output "$GITHUB_OUTPUT"

    \b
        # Use a built-in template
        repokit pr-body --template bump-version --version 1.2.0 --part minor

    \b
        # With a prefix via environment variable
        GHA_PR_BODY_PREFIX="Fix formatting" repokit pr-body
    """
...
@repokit.command(short_help="Update SHA-256 checksums for binary downloads")
@argument(
    "workflow_file",
    type=file_path(exists=True, readable=True, writable=True, resolve_path=True),
)
def update_checksums_cmd(workflow_file: Path) -> None:
    ...
    Examples:
        ...
        repokit update-checksums .github/workflows/docs.yaml

    \b
        # Verify all checksums are current (no output if all match)
        repokit update-checksums .github/workflows/autofix.yaml
    """

...
@dataclass
class Config:
    """Configuration schema for ``[tool.repokit]`` in ``pyproject.toml``.

    This dataclass defines the structure and default values for repokit configuration.
...
``[tool.repokit]`` in ``pyproject.toml``, so they no longer need to be passed through
workflow metadata outputs.
"""


def load_repokit_config(
    pyproject_data: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Load ``[tool.repokit]`` config merged with ``Config`` defaults.
    ...
    user_config: dict[str, Any] = pyproject_data.get("tool", {}).get("repokit", {})
    schema = Config()
...
class Metadata:
    ...
    def config(self) -> dict[str, Any]:
        """Returns the ``[tool.repokit]`` section from ``pyproject.toml``.

        Merges user configuration with defaults from ``Config``.
        """
        return load_repokit_config(self.pyproject_toml)

    @cached_property
    def unstable_targets(self) -> set[str]:
        """Nuitka build targets allowed to fail without blocking the release.

        Reads ``[tool.repokit].unstable-targets`` from ``pyproject.toml``. Defaults
        to an empty set.
    ...
    def nuitka_matrix(self) -> Matrix | None:
        ...
        if not self.config["nuitka"]:
            logging.info(
                "[tool.repokit] nuitka is disabled. Skipping binary compilation."
            )
            return None
        ...
        # Pass project-specific Nuitka flags from [tool.repokit] config.
    ...
    def dump(
        ...
        # Add config from [tool.repokit] in pyproject.toml.

...
class RenovateCheckResult:
    ...
    def to_pr_body(self) -> str:
        ...
        docs_url = "https://github.com/kdeldycke/repokit#permissions-and-token"
        ...
        lines.append("")
        lines.append(
            "🤖 Generated with [repokit](https://github.com/kdeldycke/repokit)"
        )
...
def check_dependabot_config_absent() -> tuple[bool, str]:
    ...
    if path:
        msg = (
            ...
            "run `repokit init renovate` to get a starter config, "
...
def check_renovate_config_exists() -> tuple[bool, str]:
    ...
    msg = "renovate.json5 not found. Run `repokit init renovate` to create it."
    return False, msg
...
``repokit = { timestamp = "<ISO datetime>", span = "PT0S" }`` from the

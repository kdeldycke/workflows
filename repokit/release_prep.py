...
types, delegating changelog operations to :mod:`repokit.changelog`. It
handles:

- Changelog freeze via :meth:`Changelog.freeze() <repokit.changelog.Changelog.freeze>`.
- Setting the release date in ``citation.cff``.
- Freezing workflow URLs to versioned tags (for `kdeldycke/repokit`).
- Freezing ``repokit`` CLI invocations to PyPI versions, unfreezing back to
  local source (``--from . repokit``).
"""
...
class ReleasePrep:
    ...
    def freeze_workflow_urls(self) -> int:
        ...
        Replaces ``/repokit/{default_branch}/`` with ``/repokit/v{version}/``
        and ``/repokit/.github/actions/...@{default_branch}`` with
        ``/repokit/.github/actions/...@v{version}`` in all workflow YAML files.
        ...
        count = 0
        # URL pattern: /repokit/main/ -> /repokit/v1.2.3/
        url_search = f"/repokit/{self.default_branch}/"
    ...
    def _json5_files(self) -> list[Path]:
        ...
        return [
            ...
            repo_root / "repokit" / "data" / "renovate.json5",
        ]
    ...
    def freeze_cli_version(self, version: str) -> int:
        ...
        This is part of the **freeze** step: it freezes ``repokit``
        ...
        Replaces ``--from . repokit`` with ``'repokit=={version}'`` in all
        workflow YAML files, and with ``repokit=={version}`` (unquoted) in
        ...
        count = 0
        search = "--from . repokit"
        yaml_replace = f"'repokit=={version}'"
        ...
        json5_replace = f"repokit=={version}"
    ...
    def unfreeze_cli_version(self) -> int:
        ...
        This is part of the **unfreeze** step: it reverts ``repokit``
        invocations back to local source (``--from . repokit``) for the next
        development cycle on ``main``.

        Replaces ``'repokit==X.Y.Z'`` (quoted, in YAML) and
        ``repokit==X.Y.Z`` (unquoted, in ``renovate.json5``) with
        ``--from . repokit``.
        ...
        count = 0
        yaml_pattern = re.compile(r"'repokit==[\d.]+'")
        replace = "--from . repokit"
        ...
        json5_pattern = re.compile(r"repokit==[\d.]+")
        for json5_file in self._json5_files:
            if not json5_file.exists():
                logging.debug(f"JSON5 file not found: {json5_file}")
                continue
    ...
    def unfreeze_workflow_urls(self) -> int:
        ...
        Replaces ``/repokit/v{version}/`` with ``/repokit/{default_branch}/``
        and ``/repokit/.github/actions/...@v{version}`` with
        ``/repokit/.github/actions/...@{default_branch}`` in all workflow YAML files.
        ...
        count = 0
        # URL pattern: /repokit/v1.2.3/ -> /repokit/main/

...
files in ``repokit/templates/``, optionally with YAML frontmatter for
...
def load_template(name: str) -> tuple[dict[str, object], str]:
    """Load a PR body template from the ``repokit/templates/`` package.
    ...
    """
    template_files = files("repokit.templates")
...
def render_template(name: str, **kwargs: str) -> str:
    ...
    conflicts with markdown braces like ``[tool.repokit]``.
...
def get_template_names() -> list[str]:
    ...
    """
    template_dir = files("repokit.templates")
    names = []

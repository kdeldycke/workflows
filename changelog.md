...
## [`6.13.0.dev0` (unreleased)](https://github.com/kdeldycke/repomatic/compare/v6.12.0...main)
...
- Link binary names in the VirusTotal scans release section to their GitHub release download URLs.
- Add two-phase VirusTotal scanning: phase 1 uploads binaries and writes an initial table with scan links, phase 2 (`--poll`) polls for analysis completion and replaces the table with a Detections column showing `flagged / total` engine counts. The workflow uses two sequential steps so users see scan links immediately while detection stats populate asynchronously.
...
- Normalize all workflow token references to `GH_TOKEN` with `github.token` as the default. `GITHUB_TOKEN` is not an automatic environment variable in GitHub Actions (it is only available via expression syntax), so all steps that call `gh` or repomatic commands must set `GH_TOKEN` explicitly. Replace `secrets.GITHUB_TOKEN` with `github.token` everywhere.
- Fix missing `GH_TOKEN` in `sync-dev-release`, `sponsor-label`, `setup-guide`, `lint-repo`, `check-renovate`, and `scan-virustotal` workflow steps.
- Add pre-flight `shutil.which` check in `run_exiftool` for a clear error when exiftool is not installed, instead of a bare `FileNotFoundError`.
- Verify exiftool is on PATH after `choco install` in the Windows binary build step, catching silent Chocolatey CDN failures.

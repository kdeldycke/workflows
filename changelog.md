...
- Pin the `mdformat-pelican` dependency to a fork commit that fixes a link-renderer conflict with `mdformat-recover-urls` under `mdformat-gfm` 1.0.0. Without the fix, plugin loading order on Linux let `mdformat-pelican`'s fallback link renderer win, leaving non-ASCII anchor links (e.g. Chinese, accented characters) percent-encoded on every `format-markdown` run. The fork replaces the renderer override with a postprocessor; pending upstream merge at [`gaige/mdformat-pelican` PR #11](https://github.com/gaige/mdformat-pelican/pull/11).

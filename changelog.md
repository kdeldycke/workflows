...
- Fix `update-docs` ↔ `format-markdown` ping-pong on `docs/cli.md`. The `cli_reference()` generator emitted a blank line between the seed `{click:source}` directive's `:hide-source:` option and its source line, but `mdformat-myst` rewrites the option as a YAML block and the post-processor in `repomatic.tool_runner._fix_myst_directive_options` converts it back to field-list syntax without the blank, so the two tools alternated insertion and removal on every run.
- Fix `update-docs` ↔ `format-markdown` ping-pong on `docs/configuration.md`. Triple-backtick fences in MyST docstrings were mangled by the `.replace()` post-processing in `config_reference()` and `config_full_descriptions()`, causing the two tools to undo each other's output on every run.

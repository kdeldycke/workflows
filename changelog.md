...
- Bump pinned `uv` from `0.11.6` to `0.11.8` in `renovate.json5` and the bundled `repomatic/data/renovate.json5` template. Drop the lock-file noise-detection layer (`is_lock_diff_only_timestamp_noise`, `revert_lock_if_noise`, `_TIMESTAMP_LINE_RE`, `SyncResult.reverted`) now that [astral-sh/uv#19022](https://github.com/astral-sh/uv/pull/19022) and [astral-sh/uv#19101](https://github.com/astral-sh/uv/pull/19101) make `uv lock` write the `0001-01-01T00:00:00Z` sentinel for relative `exclude-newer` spans, eliminating the per-run timestamp churn that motivated those workarounds.
- Bump `mdformat-pelican` to `1.0.0`, the first release that ships the link-renderer fix from [`gaige/mdformat-pelican` PR #11](https://github.com/gaige/mdformat-pelican/pull/11). Without the fix, plugin loading order on Linux let `mdformat-pelican`'s fallback link renderer win under `mdformat-gfm` 1.0.0, leaving non-ASCII anchor links (like Chinese or accented characters) percent-encoded on every `format-markdown` run. The release replaces the renderer override with a postprocessor.
- Harmonize the `sphinx-docs` agent and `sphinx-docs-sync` skill with conventions surveyed across `click-extra`, `extra-platforms`, and `meta-package-manager`. The agent now carries a canonical extension set (with notes on `sphinx_click` vs `click_extra.sphinx`), an octicon registry for standard page titles, governance rules for `suppress_warnings`/`nitpick_ignore`/`linkcheck_ignore` entries, the descriptive `<!-- {feature}-{kind}-start -->` auto-region marker convention, theme-asset paths cross-referenced with the `/brand-assets` skill, and a Sphinx-tests recipe pattern for cross-reference render tests. The sync skill's audit checklist gains matching categories. `docs/myst-docstrings.md` documents the concrete failure mode (backtick-doubled `:py:obj:` roles) when `repomatic.myst_docstrings` loads after `sphinx_autodoc_typehints`.
- Drop `sphinx_issues` from the canonical extension set and add a "Migrating off `sphinx_issues`" recipe to the agent. The extension's `:issue:`/`:pr:`/`:user:`/`:commit:` roles render only inside Sphinx, so the same source files (changelogs, docstrings) display as raw role text in GitHub's repo browser, IDE previews, `pip show`, and the PyPI long-description. Replacing the roles with explicit GitHub URLs (`[#N](https://github.com/{owner}/{repo}/issues/N)`) keeps the changelog identically readable on every renderer. The recipe ships a Python migration script that handles MyST roles, reST roles, and cross-repo prefixed forms. The `sphinx-docs-sync` skill grows a matching audit category that flags every occurrence.
...
- Fix the Sphinx build crashing in `docs/cli.md` with `NameError: name 'repomatic' is not defined`. `{click:run}` calls `exec(code, namespace, local_vars)`, so a top-level `import` inside the block lands in `local_vars` and is discarded after the block runs. The shared `repomatic` symbol is now seeded once via a leading `{click:source}` `:hide-source:` block, which calls `exec(code, namespace)` and persists assignments to the per-document runner namespace.
- Fix mypy `[no-any-return]` errors in `docs/docs_update.py` `_toml_key()` by typing the dataclass field parameter as `dataclasses.Field` and casting the metadata lookup result with `str(path)`, mirroring the pattern already in `repomatic/config.py`.
...
- Fix broken documentation links in PR body templates. Links pointing at `readme.md` anchors (`?tab=readme-ov-file#toolrepomatic-configuration`, `#githubworkflowsautofixyaml-jobs`, `#githubworkflowschangelogyaml-jobs`, `#githubworkflowsreleaseyaml-jobs`, `#example-usage`) silently scrolled to the top of the readme since those sections moved out into the Sphinx site. All 18 templates under `repomatic/templates/` now point at `kdeldycke.github.io/repomatic/configuration.html` and the matching `workflows.html` anchors. Each `### Configuration` block was reshaped from an opaque TOML dump into a sorted bulleted list where every option name links to its own anchor on the configuration page (e.g. `[`gitignore.location`](configuration.html#gitignore-location)`), giving readers direct access to the type, default, and example for each option without duplicating that content into the PR body.
- Fix `update-docs` ↔ `format-markdown` ping-pong on `docs/configuration.md`. The ``` .replace("``", " ```")`post-processing in`config_reference()`and`config_full_descriptions()```` mangled triple-backtick fences in MyST docstrings (`` ```toml `` → `` ``toml ``), and an indented-code-block example plus an inline-code span broken across lines round-tripped differently through Sphinx and ````mdformat`. Docstrings in `repomatic/config.py`are now MyST single-backtick throughout, the workflow-paths example uses a fenced`toml\` block, and the regen → format → regen pipeline is a fixed-point.

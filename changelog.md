...
- Add editorconfig serialization to the tool runner, allowing `[tool.X]` sections in `pyproject.toml` to be translated for CWD-discovery tools that have no `--config` flag (like `shfmt`). The translated config is written to the tool's native config path, used for the invocation, then cleaned up.
- Deduplicate the `Generated by repomatic ...` file header into a shared `GENERATED_HEADER_TEMPLATE` constant and `generated_header()` function in `tool_runner.py`. Both CLI commands (e.g. `sync-mailmap`) and tool runner config translation now use the same template, with a UTC timestamp on every generated file.
...
- Add global binary cache for downloaded tool executables. `repomatic run` now caches binaries under a platform-appropriate user cache directory (`~/Library/Caches/repomatic` on macOS, `~/.cache/repomatic` on Linux) so repeated invocations skip the download. Cached binaries are re-verified against their registry SHA-256 checksum on every use. Add `repomatic cache show`, `repomatic cache clean`, and `repomatic cache path` subcommands for cache management. Add `--no-cache` flag to `repomatic run` to bypass the cache. Stale entries are auto-purged after 30 days (configurable via `REPOMATIC_CACHE_MAX_AGE` env var).
- Add HTTP response cache for PyPI metadata (24h TTL) and GitHub release bodies (24h for all-releases, 7d for single-release). Avoids redundant API calls across `changelog`, `sync-uv-lock`, and `sync-github-releases` invocations. Cached responses are checked for freshness on every use via file mtime. Add `--namespace` option to `repomatic cache clean` for targeted cleanup.
...
- Fix `format-json` autofix job failing with `flag --config-path is not valid in this context` when a `[tool.biome]` section exists in `pyproject.toml`. Biome's CLI parser (bpaf) scopes global options like `--config-path` inside the subcommand, so `biome format --config-path X` is valid but `biome --config-path X format` is not. Add `config_after_subcommand` field to `ToolSpec` that splices config args after the first token of `extra_args` (the subcommand name) instead of before it.
...
- Sanitize `@mentions`, `#issue` references, and `github.com` URLs in upstream release notes embedded in `sync-uv-lock` PR bodies. Inserts Unicode zero-width spaces to prevent GitHub from auto-linking mentions (which spams unrelated users with notifications) and issue references (which can accidentally close downstream issues on merge). Rewrites `github.com` URLs to `redirect.github.com` to prevent backlink cross-references on upstream issues. Code fences and inline code spans are preserved via placeholder extraction. Closes #2542.
- Route generated tool configs through the cache directory (`~/.cache/repomatic/config/{tool}/{filename}`) instead of writing temp files to `/tmp` or polluting the user's repository with CWD-discovery files. All tools with a `--config` flag now receive an explicit `--config <cache_path>` argument, making repomatic's involvement visible in logs and avoiding cleanup issues from interrupted runs. Tools without a `--config` flag (mdformat, shfmt) remain CWD-discovery exceptions. Add `config/` subtree to the cache module with `store_config()`, `config_cache_info()`, and `clear_config_cache()`. Remove the `__bundled__` sentinel from `run_tool()`: bundled defaults are now eagerly resolved into the cache like any other generated config.

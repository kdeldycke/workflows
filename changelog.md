...
- Add Sphinx documentation site with Furo theme, MyST-Parser, and 13 extensions. Split the monolithic `readme.md` (1139 lines) into focused pages: installation, configuration reference (auto-generated from `Config` dataclass docstrings via `docs/docs_update.py`), CLI parameters (auto-generated via `click_extra.sphinx`), reusable workflow reference, security practices, and Claude Code skills. Deployed to GitHub Pages via the existing `docs.yaml` workflow.
- Add dedicated tool runner documentation page with a tutorial. Covers the 4-level config precedence chain, available tools table, `[tool.X]` bridge, binary caching, version overrides, and a step-by-step yamllint walkthrough.
- Add `repomatic.myst_docstrings` Sphinx extension and `repomatic.myst_converter` utility module. The extension hooks into `autodoc-process-docstring` to transparently convert MyST markdown syntax in Python docstrings to reST at Sphinx build time, so `sphinx.ext.autodoc` works without modification. The converter is a standalone migration utility that rewrites Python source files from reST to MyST docstring syntax in place. Both are idempotent.
